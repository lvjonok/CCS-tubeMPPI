...
def rollout_trajectory(
    x0, T, dt, F, Ubar, EpsSetk, C, Sigmainv, lambda_, Nu_MPPIinv, Phi
):
    xt = x0
    Sk = 0.0
    for t in range(T):
...
def MPPI_pathos(
    ...
    for k in range(K):
        EpsSetk = np.random.multivariate_normal(np.zeros(nu), Sigma_MPPI, size=(T)).T
        if k == 0:
            EpsSetk = EpsSetk * 0.0
        EpsSetAllk.append(EpsSetk)

    # Create function handle for trajectory rollout function
    rollout_handle = lambda x, eps_set: rollout_trajectory(
        x, T, dt, F, Ubar, eps_set, C, Sigmainv, lambda_, Nu_MPPIinv, Phi
    )
    ...
    for t in range(T):
        ...
        X[:, t : t + 1] = xbart
        utm1 = Ubar[:, t : t + 1]
        Sout += C(xbart) + (lambda_ / 2.0) * (ubartm1.T @ Sigmainv @ utm1)
...
def MPPI_thread(
    ...
    rollout_q = Queue()
    cost_q = Queue()
    process_workers = [rolloutWorker(rollout_q, cost_q) for i in range(num_workers)]
    ...
    for k in tqdm(range(K), disable=not progbar):
        ...
        rollout_q.put((x0, T, dt, Ubar, EpsSetk, Sigmainv, lambda_, Nu_MPPIinv, k))
...
def MPPI(x0, F, K, T, Sigma, Phi, C, lambda_, Ubar, Nu_MPPI=1.0, dt=0.1, progbar=False):
    """
    Nominal Controller MPPI Function:
    Inputs:
    x0 : Initial State (numpy array (nx,1))
